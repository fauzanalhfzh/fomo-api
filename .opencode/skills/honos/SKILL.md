---
name: honos
description: Contains the complete Hono framework documentation inline. Use when building Hono routes, middleware, validators, or any Hono API code.
---
...
### âŒ Don't: Try to Create Dedicated HEAD Handlers
...
app.get('/bar', (c) => {
  const val = c.get('result') // âŒ val is undefined but typed as a string, which can lead to runtime errors
})
```
